...
from computer_vision.VisionMBR import VisionMBR
...
class ClientNaiveAgent(Thread):
	...
	def run(self):
		...
		while True:
			...
			if state == GameState.WON:
				...
				, self.current_level)
				ar.load_level(self.current_level)
	...
	def solve(self):
		...
		* @return GameState: the game state after shots.
		...
		# process image
		vision = VisionMBR(screenshot)
		
		sling = list(vision.find_slingshot_mbr()['slingshot'])[0]
		...
		if sling != None:
			...
			if len(pigs)!=0:
				...
				screenshot = self.ar.do_screenshot()
				vision = VisionMBR(screenshot)
				_sling = list(vision.find_slingshot_mbr()['slingshot'])[0]
				if _sling != None:
